	...
	switch (req.url) {
		...
		case '/':
            ...
            movies.forEach(movies =>{
                ...
                res.write('\n\n')
                movies.sort((a, b) => (a.title > b.title)?1:(a.title < b.title)?-1:0); //ordena un array alfabeticamente por una propiedad del mismo
		...
		case '/en-cartelera':
            ...
            res.write('\n\n')
            movies.forEach(movies =>{
                res.write('- ' + movies.title + ' ' + movies.overview)
                res.write('\n\n');
                })
		...
		case '/mas-votadas':
			...
			res.write('\n\n')
			let voteSiete = movie.filter((movies)=>{return movies.vote_average >= 7});	  
			res.write('Total de peliculas : ' + ' esta pelicula tiene calificación : ' + voteSiete.length + 'reseña : ' + movies.overview);
			res.end('Más Votadas');
		...
		case '/sucursales':
			...
			res.write('\n\n')
			res.write ('SALAS EXCLUSIVAS PARA UNA EXPERIENCIA UNICA : ' + theaters.length);
			res.write('\n\n')
			theaters.forEach(theaters=>{
				res.write('- ' + theaters.name + ' NOS ENCONTRAS EN : ' + theaters.address + ' Y CONOCÉ : ' + theaters.description)
				res.write('\n\n')
			})
			res.end('Sucursales');
			break;
		...
		case '/preguntas-frecuentes':
			...
			res.write('\n\n')
			res.write('PREGUNTAS : ' + faqs.length);
			res.write('\n\n')
            faqs.forEach(faqs =>{
